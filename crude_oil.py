...
def quantity_by_year_bar():
    ...
    As we already have quantati measured in thousands barrels.
    So, diagram will show quantuty in billions barrels.
    """
    ...
    cm.make_bar(x_labels, y_values,
                title="Quantity of imported oil by years,(measured in billions of barrels)")
...
def type_oil_for_last_5_years():
    ...
    Rounded to quantity to millions.As we already have quantati measured in thousands barrels.
    So, diagram will show quantuty in billions barrels.
    """
    type_oil_dict = {}
    for row in ARR[1:]:
    ...
    for key, var in type_oil_dict.items():
        type_oil_dict[key] = sum(var) / 1000000
    x_vals = list(type_oil_dict)
    y_vals = []
    for lab in x_vals:
        y_vals.append(type_oil_dict[lab])
    _, axis = plt.subplots(figsize=(8, 4))
    axis.set_title('Quantity of imported oil by types from 2020(measured in billions of barrels)')
...
def type_oil_new():
    ...
    if selected_year:
        ...
        cm.make_pi(labels, values, title=f" Percentage distribution of quantity imported oil for{selected_year}")
